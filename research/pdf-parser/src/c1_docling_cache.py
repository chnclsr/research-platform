...
KORPUS_KOK = os.path.abspath(os.environ.get(
    "KORPUS_RAW", os.path.join(os.path.dirname(BASE), "research-platform", "corpus", "raw")
))
URETIM_SRC = os.path.abspath(os.environ.get(
    "RESEARCH_PLATFORM_SRC", os.path.join(os.path.dirname(BASE), "research-platform", "src")
))
...
def _resolve(record: dict) -> str:
    root = BASE if record.get("yol_koku") == "staj" else KORPUS_KOK
