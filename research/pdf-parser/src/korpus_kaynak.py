...
#: STAJ = research-platform/research/pdf-parser (bu dosyanin bulundugu proje
#: koku); DEPO_KOKU iki ust dizine cikip research-platform'un kendisini bulur
#: -- 2026-08-20'den once bu dosya sude-staj'da, research-platform'un KARDESI
#: bir depoda yasiyordu, o yuzden tek ust dizin yeterliydi. Artik
#: research-platform'un ICINDE oldugu icin iki ust dizin gerekiyor.
STAJ = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
DEPO_KOKU = os.path.dirname(os.path.dirname(STAJ))
KORPUS_KOK = os.path.abspath(os.environ.get(
    "KORPUS_RAW",
    os.path.join(DEPO_KOKU, "corpus", "raw"),
))
...
def _staj_goreli(yol: str) -> str:
    return os.path.relpath(yol, STAJ).replace("\\", "/")
...
# CODEX-2026-08-18: 200 Ingilizce PDF + ground-truth Markdown ciftini C1'e
# baglar. 2026-08-20'ye kadar bu veri yalniz sude-staj/tools/opendataloader-bench
# altindaydi (STAJ'a gore, kopyalanmadan); "tek dogru yer" icin diger uc
# korpusla (ocrturk, doclaynet_icdar, omnidocbench) ayni konvansiyona tasindi:
# KORPUS_KOK/opendataloader_bench. 173 MB, corpus/raw/ gibi gitignore'da.
class OpenDataLoaderBenchKaynak:
    ad = "opendataloader_bench"

    def __init__(self, kok: Optional[str] = None, hash_hesapla: bool = True):
        self.kok = kok or os.path.join(KORPUS_KOK, "opendataloader_bench")
    ...
    def kayitlar(self) -> Iterator[KorpusKaydi]:
        ...
        for name in sorted(os.listdir(self.pdf_kok)):
            ...
            record = KorpusKaydi(
                veri_seti=self.ad, kimlik=stem, yol_koku="staj",
                yetenekler=[PDF_PARSE], pdf_yolu=_staj_goreli(pdf),
            ...
            if os.path.isfile(md):
                ...
                record.referans = {
                    "bicim": "markdown", "yol": _staj_goreli(md),
