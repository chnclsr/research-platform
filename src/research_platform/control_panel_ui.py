...
function collapsibleDetailSection(title,open=false){const wrap=h('details','drawer-section collapsible-section'),head=h('summary','section-title'),content=h('div','section-content');head.append(h('h3','',title));wrap.append(head,content);wrap.open=open;return{wrap,content}}
...
const planLabels={
 tr:{questions:'Sorular',queryPlan:'Sorgu planı',sourceSelection:'Kaynak seçimi',dateScope:'Tarih kapsamı',limits:'Bütçe ve etkin sınırlar',models:'Modeller',acquisition:'Edinim ve ayrıştırma',deliverables:'Çıktılar',checkpoints:'Kalan onay noktaları',strategy:'Strateji notu',feedback:'Önceki geri bildiriminiz',revision:'revizyon',researchWording:'Araştırma dili (İngilizce)',
  ...
  duration:'Araştırma süresi (dakika)',approve:'Onayla ve başlat',reject:'Değişiklik iste',rejectNeedsReason:'Değişiklik isterken gerekçe yazın; plan onu kullanarak yeniden kuruluyor.',
 ...
 en:{questions:'Questions',queryPlan:'Query plan',sourceSelection:'Source selection',dateScope:'Date scope',limits:'Budget and effective limits',models:'Models',acquisition:'Acquisition and parsing',deliverables:'Deliverables',checkpoints:'Remaining checkpoints',strategy:'Strategy note',feedback:'Your earlier feedback',revision:'revision',researchWording:'Research wording (English)',
  ...
  duration:'Research duration (minutes)',approve:'Approve and start',reject:'Request changes',rejectNeedsReason:'Write why you are rejecting; the plan is rebuilt from that note.',
...
const rows=stage.tools||[];if(!rows.length){box.append(h('div','empty','Bu aşamada araç çağrısı kaydedilmedi.'));return}
...
function roundSummaryText(visit){const s=visit.summary||{},parts=[];if(s.connectors)parts.push(`${s.connectors} connector`);if(s.calls)parts.push(`${s.calls} çağrı`);if(s.tokens)parts.push(`${s.tokens} tok`);if(s.errors)parts.push(`${s.errors} hata`);return parts.join(' · ')||'işlem yok'}
...
function toggle(){const open=detail.hidden;detail.hidden=!open;tr.classList.toggle('selected',open);tr.setAttribute('aria-expanded',open?'true':'false');if(open&&!cell.childNodes.length){const inner=h('div');renderStageTools(inner,visit,toggle);cell.append(inner)}}
tr.onclick=toggle;tr.onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();toggle()}};tbody.append(tr,detail)}
table.append(thead,tbody);wrap.append(table);box.append(wrap);
if(payload.has_more)box.append(h('div','note',`İlk ${visits.length} tur gösteriliyor; toplam ${payload.visit_count}.`))}
...
if(!stageRounds[next]){roundsBox.className='stage-hint';roundsBox.textContent='Turlar yükleniyor…';try{stageRounds[next]=await api(`/api/runs/${data.run.id}/stages/${encodeURIComponent(next)}`)}catch(e){roundsBox.textContent=e.message;return}}
...
const keep=selectedFlowStage;selectedFlowStage=null;selectStage(keep);
return section}
...
if(interaction.type==='planning_questions'){const inputs=[];for(const item of payload.questions||[]){const row=h('div','hitl-question'),label=h('label','',item.question);row.append(label);
// Options when the model produced them, free text when it did not: the same answer shape
// goes back either way, so the checkpoint keeps working for both. A fixed question also
// ships `values`, and the picked one travels back as `value` -- that is what makes the
// answer set a protocol field instead of only steering the prompts. Typing an answer
// deliberately sends no value: there is nothing to bind an invented phrase to.
const options=item.options||[],values=item.values||[];if(options.length){const group=h('div','hitl-options');options.forEach((option,n)=>{const choice=h('label','hitl-option'),radio=document.createElement('input');radio.type='radio';radio.name=`q:${inputs.length}`;radio.value=option;radio.dataset.bound=values[n]||'';choice.append(radio,h('span','',option));group.append(choice)});const other=document.createElement('input');other.type='text';other.className='hitl-other';other.placeholder='Başka (yazın)';group.append(other);row.append(group);inputs.push({question:item.question,id:item.id||'',read:()=>{const typed=other.value.trim();if(typed)return{answer:typed,value:''};const picked=group.querySelector('input[type=radio]:checked');return{answer:picked?picked.value:'',value:picked?picked.dataset.bound:''}}})}
else{const input=h('textarea');input.placeholder='Yanıtınızı yazın';row.append(input);inputs.push({question:item.question,id:item.id||'',read:()=>({answer:input.value.trim(),value:''})})}
card.append(row)}const actions=h('div','hitl-actions'),send=h('button','btn primary','Yanıtları gönder');send.onclick=()=>submitHitl(data.run.id,interaction.interaction_id,{answers:inputs.map(item=>{const picked=item.read();return{question:item.question,answer:picked.answer,id:item.id,value:picked.value}})});actions.append(send);card.append(actions)}
else if(interaction.type==='plan_review'){const plan=payload.plan||{},t=planText(plan);card.append(planView(plan));const minutes=h('div','plan-duration'),label=h('label','',t.duration),input=document.createElement('input');input.type='number';input.min='1';input.max='1440';input.value=String(plan.budget?.max_wall_minutes??'');minutes.append(label,input);const guidance=h('textarea');guidance.placeholder=t.reject+'…';const actions=h('div','hitl-actions'),approve=h('button','btn primary',t.approve),reject=h('button','btn danger',t.reject);
approve.onclick=()=>{const response={approved:true};const value=Number(input.value);if(Number.isInteger(value)&&value>=1&&value<=1440&&value!==plan.budget?.max_wall_minutes)response.max_wall_minutes=value;submitHitl(data.run.id,interaction.interaction_id,response)};
reject.onclick=()=>{const note=guidance.value.trim();if(!note){toast(t.rejectNeedsReason,true);return}submitHitl(data.run.id,interaction.interaction_id,{approved:false,modifications:note})};
actions.append(approve,reject);card.append(minutes,guidance,actions)}
else if(interaction.type==='outline_review'){const view=h('div','hitl-data');view.append(h('code','',JSON.stringify(payload.outline||payload,null,2)));const guidance=h('textarea');guidance.placeholder='Değişiklik talebiniz (reddediyorsanız)…';const actions=h('div','hitl-actions'),approve=h('button','btn primary','Onayla'),reject=h('button','btn danger','Değişiklik iste');approve.onclick=()=>submitHitl(data.run.id,interaction.interaction_id,{approved:true});reject.onclick=()=>submitHitl(data.run.id,interaction.interaction_id,{approved:false,modifications:guidance.value.trim()});actions.append(approve,reject);card.append(view,guidance,actions)}
...
async function openRun(id){if(currentRunId!==id){selectedFlowStage=null;stageRounds={}}currentRunId=id;el('drawer-backdrop').classList.add('open');el('drawer-backdrop').setAttribute('aria-hidden','false');el('drawer-title').textContent=id;el('drawer-body').replaceChildren(h('div','empty','Araştırma verileri yükleniyor…'));try{renderDetail(await api(`/api/runs/${id}/detail`))}catch(e){el('drawer-body').replaceChildren(h('div','empty',e.message));toast(e.message,true)}}
function closeRun(){currentRunId=null;selectedFlowStage=null;flowFullscreen=false;stageRounds={};el('drawer-backdrop').classList.remove('open');el('drawer-backdrop').setAttribute('aria-hidden','true')}
