...
function planView(plan){const wrap=h('div','plan-view'),q=plan.questions||{},scope=plan.date_scope||{},sel=plan.source_selection||{},models=plan.models||{},acq=plan.acquisition||{},out=plan.deliverables||{};
const questions=h('div');questions.append(h('div','plan-primary',q.primary||'—'));const subs=q.sub_questions||[];if(subs.length){const list=h('ul','plan-list');for(const item of subs)list.append(h('li','',item));questions.append(list)}wrap.append(planBlock('Sorular',questions));
const branches=plan.query_plan||[];if(branches.length){const box=h('div','table-wrap'),table=h('table'),thead=h('thead'),hr=h('tr');for(const x of ['Dal','Sorgu','Aile','Sonuç sınırı'])hr.append(h('th','',x));thead.append(hr);const tbody=h('tbody');for(const row of branches){const tr=h('tr');tr.append(textCell(row.branch_id),textCell(row.query),textCell(row.required_family||'—'),textCell(row.result_limit??'—'));tbody.append(tr)}table.append(thead,tbody);box.append(table);wrap.append(planBlock('Sorgu planı',box))}
wrap.append(planBlock('Kaynak seçimi',planPairs([['Profil',sel.profile],['Aileler',(sel.included_families||[]).join(', ')],['Dışlanan connector',(sel.excluded_connectors||[]).join(', ')],['Zorunlu connector',(sel.required_connectors||[]).join(', ')],['Atıf derinliği',sel.citation_depth]])));
const dates=h('div');dates.append(planPairs([['Başlangıç',scope.start_date||'sınır yok'],['Bitiş',scope.end_date||'sınır yok'],['Coğrafya',(scope.geography||[]).join(', ')]]));if(scope.inferred_from_question)dates.append(h('div','plan-warn','Bu tarih aralığı sorunun metninden otomatik çıkarıldı; her sorguyu daraltır.'));wrap.append(planBlock('Tarih kapsamı',dates));
const limitsBox=h('div','table-wrap'),limitsTable=h('table'),limitsHead=h('thead'),limitsRow=h('tr');for(const x of ['Sınır','Değer','Bağlayıcı','Not'])limitsRow.append(h('th','',x));limitsHead.append(limitsRow);const limitsBody=h('tbody');for(const row of plan.effective_limits||[]){const tr=h('tr');tr.append(textCell(row.limit),textCell(row.value??'—'),textCell(row.binding?'evet':'hayır'),textCell(row.note));limitsBody.append(tr)}limitsTable.append(limitsHead,limitsBody);limitsBox.append(limitsTable);wrap.append(planBlock('Bütçe ve etkin sınırlar',limitsBox));
wrap.append(planBlock('Modeller',planPairs([['LLM',models.llm],['Embedding',models.embedding],['Görsel',models.vision],['Bağlam penceresi',models.context_tokens]])));
wrap.append(planBlock('Edinim ve ayrıştırma',planPairs([['Strateji sırası',(acq.strategy_order||[]).join(' → ')],['Parser',(acq.parsers||[]).join(', ')],['Parser override',Object.entries(acq.parser_overrides||{}).map(([k,v])=>`${k}=${v}`).join(', ')]])));
wrap.append(planBlock('Çıktılar',planPairs([['Teslimat',out.output_mode],['Rapor dili',out.report_language],['Paketler',(out.bundles||[]).join(', ')],['Not',out.note]])));
const checkpoints=plan.remaining_checkpoints||[];wrap.append(planBlock('Kalan onay noktaları',planPairs([['Duraklar',checkpoints.length?checkpoints.join(', '):'yok — onaydan sonra kesintisiz çalışır']])));
if(plan.strategy_note)wrap.append(planBlock('Strateji notu',h('div','plan-strategy',plan.strategy_note)));
const feedback=plan.feedback||[];if(feedback.length){const list=h('ul','plan-list');for(const item of feedback)list.append(h('li','',item));wrap.append(planBlock(`Önceki geri bildiriminiz (${plan.revision}. revizyon)`,list))}
return wrap}
...
else if(interaction.type==='plan_review'){const plan=payload.plan||{};card.append(planView(plan));const minutes=h('div','plan-duration'),label=h('label','','Araştırma süresi (dakika)'),input=document.createElement('input');input.type='number';input.min='1';input.max='1440';input.value=String(plan.budget?.max_wall_minutes??'');minutes.append(label,input);const guidance=h('textarea');guidance.placeholder='Değişiklik talebiniz (reddediyorsanız)…';const actions=h('div','hitl-actions'),approve=h('button','btn primary','Onayla ve başlat'),reject=h('button','btn danger','Değişiklik iste');
...
reject.onclick=()=>{const note=guidance.value.trim();if(!note){toast('Değişiklik isterken gerekçe yazın; plan onu kullanarak yeniden kuruluyor.',true);return}submitHitl(data.run.id,interaction.interaction_id,{approved:false,modifications:note})};
