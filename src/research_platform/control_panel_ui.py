...
let refreshing=false,currentRunId=null,lastStatus=null,selectedStageIndex=null;
...
function renderStageTools(box,stage,onClose){box.replaceChildren();if(!stage){box.className='stage-hint';box.textContent='Bir aşamaya tıklayın: o aşamada çalışan connector, parser ve modeller listelenir.';return}
...
const timelineSection=detailSection('Pipeline zaman çizelgesi');const timeline=h('div','timeline'),toolsBox=h('div','stage-hint'),cards=[],stages=data.timeline||[];
function selectStage(index){selectedStageIndex=(index===null||index===selectedStageIndex)?null:index;cards.forEach((card,i)=>{const on=i===selectedStageIndex;card.classList.toggle('selected',on);card.setAttribute('aria-expanded',on?'true':'false')});renderStageTools(toolsBox,selectedStageIndex===null?null:stages[selectedStageIndex],()=>selectStage(null))}
for(const[index,stage]of stages.entries()){const item=h('div',`stage${stage.active?' active':''}`);item.tabIndex=0;item.setAttribute('role','button');item.setAttribute('aria-expanded','false');item.append(h('div','stage-name',stage.stage),h('div','stage-meta',`Tur ${stage.round} · ${duration(stage.duration_seconds)}`),h('div','stage-meta',fmt(stage.started_at)));item.onclick=()=>selectStage(index);item.onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();selectStage(index)}};cards.push(item);timeline.append(item)}
if(!cards.length)timeline.append(h('div','empty','Henüz aşama olayı yok.'));timelineSection.append(timeline,toolsBox);body.append(timelineSection);
// A drawer refresh re-renders from scratch; reopening the stage the viewer was reading
// beats snapping them back to the top of a 90-card strip.
const keep=selectedStageIndex;selectedStageIndex=null;if(keep!==null&&keep<stages.length)selectStage(keep);else renderStageTools(toolsBox,null,()=>selectStage(null));
...
async function openRun(id){if(currentRunId!==id)selectedStageIndex=null;currentRunId=id;el('drawer-backdrop').classList.add('open');el('drawer-backdrop').setAttribute('aria-hidden','false');el('drawer-title').textContent=id;el('drawer-body').replaceChildren(h('div','empty','Araştırma verileri yükleniyor…'));try{renderDetail(await api(`/api/runs/${id}/detail`))}catch(e){el('drawer-body').replaceChildren(h('div','empty',e.message));toast(e.message,true)}}
function closeRun(){currentRunId=null;selectedStageIndex=null;el('drawer-backdrop').classList.remove('open');el('drawer-backdrop').setAttribute('aria-hidden','true')}
