...
return wrap}
let detailLoads=[],detailEpoch=0,drawerBusy=false,drawerStatus='',visitUi={},stageDepth={},openedVisits=new Set(),openDiagnosticEvents=new Set(),openDrawerSections=new Set(),openedSourceTraces=new Set(),sourceTraceFilter='all';
...
function diagnosticFields(value,depth=0){const box=h('div','trace-facts');if(value===null||typeof value!=='object'){box.append(h('span','',diagValue(value)));return box}
for(const [key,v]of Object.entries(value)){if(['schema_version','visit_id'].includes(key))continue;const label=diagLabels[key]||key;if(v&&typeof v==='object'){const d=h('details','event');d.append(h('summary','',label),diagnosticFields(v,depth+1));box.append(d)}else{const row=h('div','trace-claim');row.append(h('strong','',`${label}: `),document.createTextNode(diagValue(v)));box.append(row)}}return box}
...
card.append(diagnosticFields(p));const sourceIds=[...new Set([p.source_id,...(Array.isArray(p.evidence)?p.evidence.map(x=>x.source_id):[])].filter(Boolean))];for(const id of sourceIds){const b=h('button','btn small','Kaynak ve kanıt izini aç');const target=h('div');b.onclick=async()=>{try{const epoch=detailEpoch;const trace=await api(`/api/runs/${runId}/sources/${id}/trace`);if(epoch!==detailEpoch||runId!==currentRunId||!target.isConnected)return;target.replaceChildren(renderSourceTrace(trace))}catch(e){target.textContent=e.message}};card.append(b,target)}
const raw=h('details','event');raw.append(h('summary','','Teknik olay verisi'),h('pre','',JSON.stringify(p,null,2)));const copy=h('button','btn small','JSON kopyala');copy.onclick=async()=>{try{await navigator.clipboard.writeText(JSON.stringify(p,null,2));toast('Kopyalandı')}catch{toast('Panoya erişilemedi; metni seçerek kopyalayabilirsiniz.',true)}};raw.append(copy);card.append(raw);return card}
...
async function toggle(){const open=detail.hidden;detail.hidden=!open;tr.classList.toggle('selected',open);tr.setAttribute('aria-expanded',open?'true':'false');if(open)openedSourceTraces.add(s.id);else openedSourceTraces.delete(s.id);
...
rows.push({code:fate.code,tr,detail});sb.append(tr,detail);if(openedSourceTraces.has(s.id))toggle()}
