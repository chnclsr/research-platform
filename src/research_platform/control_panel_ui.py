...
return wrap}
let detailLoads=[],detailEpoch=0,drawerBusy=false,drawerStatus='',visitUi={},stageDepth={},openedVisits=new Set(),openDiagnosticEvents=new Set(),openDrawerSections=new Set(),openNestedDetails=new Set(),openedSourceTraces=new Set(),openInlineSourceTraces=new Set(),sourceTraceCache=new Map(),sourceTraceFilter='all';
...
setInterval(()=>{if(currentRunId&&!drawerBusy&&['queued','running','awaiting_input','paused','cancel_requested'].includes(drawerStatus)&&!document.hidden&&!document.activeElement?.matches('input,select,textarea'))openRun(currentRunId,true)},5000);
</script>
</body>
</html>"""
