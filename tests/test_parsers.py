...
def test_registry_selects_deterministically_by_document_type():
    ...
    # smart_pdf outranks pymupdf_fast / pypdf on priority; all accept "pdf", the pick stays deterministic.
...
def test_registry_supports_legacy_overrides_aliases():
    registry = build_parser_registry()
    assert registry.select("pdf", "application/pdf", b"%PDF", {"pdf": "pdf"}).id == "smart_pdf"
...
def test_registry_override_only_applies_to_the_named_document_type():
    registry = _two_html_parsers()
    assert registry.select("pdf", "application/pdf", b"%PDF", {"html": "html_alt"}).id == "smart_pdf"
